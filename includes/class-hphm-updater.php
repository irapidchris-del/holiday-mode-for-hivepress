...
if ( ! class_exists( 'Hphm_Updater' ) ) :
	...
	final class Hphm_Updater {
		...
		public function __construct( $file, $repo ) {
			...
			add_filter( 'handle_bulk_actions-plugins', [ $this, 'handle_bulk_check' ], 10, 3 );
			...
			if ( empty( $GLOBALS['hpx_update_check_bulk'] ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.
				$GLOBALS['hpx_update_check_bulk'] = 'holiday-mode-for-hivepress'; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.

				add_filter( 'bulk_actions-plugins', [ $this, 'add_bulk_check' ] );
				add_filter( 'handle_bulk_actions-plugins', [ $this, 'finish_bulk_check' ], 20, 3 );
				add_action( 'admin_notices', [ $this, 'show_bulk_check_notice' ] );
				add_action( 'network_admin_notices', [ $this, 'show_bulk_check_notice' ] );
				add_action( 'admin_print_styles-plugins.php', [ $this, 'print_plugins_screen_styles' ] );
			}
			add_action( self::CACHE_KEY . '_refresh', [ $this, 'refresh_release' ] );
		...
		public function get_latest_release( $force = false ) {
			...
			/*
			 * A cold cache must not be filled from somebody's page load. WordPress asks every plugin
			 * for its update details while rendering an admin request, so with several of these
			 * installed one such request made one blocking call to GitHub after another, in series:
			 * a site with nine of them measured 18.6 seconds on a settings screen, once, and then
			 * behaved perfectly for six hours because the answers were cached again. That is the
			 * same shape as the listing-save incident, on the admin side rather than the public one.
			 *
			 * So the fetch moves to a background job and this answers with what is already known.
			 * Nothing is skipped: the job runs moments later, and the manual Check for updates link
			 * still fetches immediately, because there a person is waiting for the answer.
			 */
			if ( ! $force ) {
				$this->schedule_release_refresh();
		 ...
		 *
		 * Prefers HivePress's scheduler, which is Action Scheduler and already refuses a duplicate of
		 * a job with the same hook and arguments, so repeated admin requests coalesce into one fetch.
		 * WP-Cron is the fallback for the same reason it exists: it also runs the work outside this
		 * request.
		 *
		 * Neither is blocking, so where cron itself is starved the cache simply stays cold and no
		 * update is offered until somebody presses Check for updates, which always fetches at once.
		 *
		 * @return void
		 */
		public function schedule_release_refresh() {
			$hook = self::CACHE_KEY . '_refresh';

			// Assigned and then tested: Core defines no __isset(), so isset( hivepress()->x ) is
			// always false even for a component that is present and working.
		 ...
		 * @return array<string, string>
		 */
		public function add_bulk_check( $actions ) {
			if ( current_user_can( 'update_plugins' ) ) {
				$actions['hpx_check_updates'] = __( 'Check for updates', 'holiday-mode-for-hivepress' );
			}
		 ...
		 * @return string
		 */
		public function handle_bulk_check( $redirect, $action, $plugin_files ) {
			if ( 'hpx_check_updates' === $action && current_user_can( 'update_plugins' ) && in_array( $this->basename, (array) $plugin_files, true ) ) {
				$this->get_latest_release( true );
			}
		 ...
		 * @return string
		 */
		public function finish_bulk_check( $redirect, $action, $plugin_files ) {
		 ...
		 * @return void
		 */
		public function show_bulk_check_notice() {
			...
			if ( $available ) {
				...
				$message = sprintf( _n( 'Checked %1$s plugin for updates: %2$s can be updated.', 'Checked %1$s plugins for updates: %2$s can be updated.', $checked, 'holiday-mode-for-hivepress' ), number_format_i18n( $checked ), number_format_i18n( $available ) );
			} else {
				/* translators: %s: number of plugins checked. */
				$message = sprintf( _n( 'Checked %s plugin for updates: it is up to date.', 'Checked %s plugins for updates: all are up to date.', $checked, 'holiday-mode-for-hivepress' ), number_format_i18n( $checked ) );
			}
		 ...
		 * @return void
		 */
		public function print_plugins_screen_styles() {
