...
* New: a live preview to the right of the settings draws the vendor banner and the profile notice
  with the wording, icon, sizes and colours on the page, following every change as you make it;
  drag its edge to make it wider. The vendor name is an example, and nothing is stored until you
  press Save Changes.
* Changed: the Chosen Roles list is shown only while Offer the Switch To says Chosen roles, the
  one choice that reads it.
* Fixed: the "Account → Settings" link in the vendor banner showed stray characters around the
  arrow.
